...
class Crawler {
  ...
  crawl() {
    return this._prepare()
      .then(() => this._request())
      .then(response => this._beforeScrape(response))
      .then(response => (
        Promise.all([
          this._scrape(),
          this._screenshot(),
   ...
   * @private
   */
  _beforeScrape(response) {
    console.log('_beforeScrape');
    if (!this._options.beforeScrape) return Promise.resolve(response);
    return this._options.beforeScrape(this._page, this._options).then(() => response);
  }
